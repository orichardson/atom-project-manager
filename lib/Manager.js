...
export class Manager {
  ...
  constructor() {
    ...
    atom.config.onDidChange('project-manager.includeGitRepositories', ({ newValue }) => {
      if (newValue) {
        this.gitStore.fetch();
      } else {
        this.gitStore.empty();
    ...
    atom.config.onDidChange(undefined, {}, () => {
      if (this.activeProject) {
        this.settings.update(this.activeProject.settings);
      }
    });
    ...
    autorun(() => {
      if (this.activeProject) {
        console.log('autorun settings load @Manager.js')
        this.settings.load(this.activeProject.settings, this.activeProject.rootPath);
      }
    });
